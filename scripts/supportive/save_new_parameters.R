...
continents <- c("Africa","Asia","Europe","Indopacific","North_America","South_America")
...
for(continent in continents){
  ...
  #save original RPP
  write.csv(parameters[,names(parameters) != "Continent"],
            paste0("output/PANGAEA/original_parameters_",continent,".csv"),
            row.names = FALSE)
  ...
  #save optimized RPP
  write.csv(parameters[,names(parameters) != "Continent"],
            paste0("output/PANGAEA/optimized_parameters_",continent,".csv"),
            row.names = FALSE)
}
