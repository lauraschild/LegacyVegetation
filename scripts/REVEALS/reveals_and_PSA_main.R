...
# INITIALISATION ===================================================================================
{
    ...
    {
        # definition for which computer names the folder pathes
        # as mounted on linux 1..5, i.e. beginning from the server root directory
        cluster_names = paste0("linux", 1:5)
        
        cluster_main_dir = file.path("/bioing", "data", "LegacyPollen", 
                                     "REVEALS_Analysis_Global")
        local_main_dir = file.path("//dmawi.de", "potsdam","data", "bioing", 
                                   "user", "lschild","reveals_and_psas")
        if (!Sys.info()['nodename'] %in% cluster_names){
            main_dir = local_main_dir
        } else{ 
            main_dir = cluster_main_dir
        }
        setwd(main_dir)
    }
...
# PREPROCESSING ====================================================================================
{
    ...
    # load continental pollen data frames ====
    {
        ...
        pollen_dfs = load_contintental_pollen_frames()
