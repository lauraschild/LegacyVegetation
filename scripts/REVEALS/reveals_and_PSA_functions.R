...
# REVEALS FUNCTIONS (partially modified from REVEALSinR 0.9.7)
    ...
    run_reveals_with_PSA = function(pollen, 
                                    params, 
                                    tBasin, 
                                    dBasin, 
                                    dwm = "lsm unstable", 
        ...
        # compute pollen deposition (i.e. pollen influx from regioncutoff)
        {
            ...
            if (use_fixed_steps_lake_model){
              ...
              look_up_K <- function(vg_taxon){
                vg_taxon <- round(vg_taxon,9)
                deposition <- K_factors %>%
                  mutate(fallspeed = round(fallspeed,9),
                         K= round(K,8)) %>%
                  filter(Basin_type == tBasin,
                         Basin_diameter == dBasin,
                         fallspeed == vg_taxon)%>%
                  pull(K)
                print(unique(deposition))
                return(unique(deposition))
              }
              deposition = do.call(rbind,(lapply(vg,
                                                look_up_K)))
